...
BEGIN {
  ...
  def search_and_replace_in_template(filepath, service)
    ...
    array_to_insert = [
      ...
      "      - \"traefik.http.routers.#{service}-http.rule=Host(`{% if #{service}.custom_domain %}{{ #{service}.custom_domain }}{% else %}{{ #{service}.subdomain + \".\" + domain }}{% endif %}`)\"",
      ...
      '{% endif %}',
      "      - \"traefik.http.routers.#{service}.rule=Host(`{% if #{service}.custom_domain %}{{ #{service}.custom_domain }}{% else %}{{ #{service}.subdomain + \".\" + domain }}{% endif %}`)\"",
