...
# First build the docker images needed to deploy
	@sudo docker inspect --type=image homelabos/homelabos:$(VERSION) > /dev/null && printf "$(byel)========== Docker image already built ==========\n$(end)" || sudo docker build . -t homelabos/homelabos:$(VERSION)
